...
import "../ERC3998.sol";
import "./IERC3998Evolvable.sol";

abstract contract ERC3998Evolvable is ERC3998, IERC3998Evolvable {
    ...
    function mintWith(
        ...
        require(
            ...
            "ERC3998Evolvable: probabilities and evolutiveIntervals length mismatch"
        );
    ...
    function attach(
        ...
        uint256 attrId,
        uint256 amount
    ) public virtual override(ERC3998, IERC3998) {
        ...
        override
    {
        require(
            ...
            "ERC3998Evolvable: token has not attached the attribute"
        );
        require(
            _states[attrId][tokenId].status,
            "ERC3998Evolvable: token has broken"
        );
        uint8 curLv = _states[attrId][tokenId].curLevel;
        require(
            curLv <= _settings[attrId].probabilities.length,
            "ERC3998Evolvable: exceeded the maximum level"
        );
        require(
    ...
    function repair(uint256 tokenId, uint256 attrId) public virtual override {
        ...
        require(
            !_states[attrId][tokenId].status,
            "ERC3998Evolvable: token is normal"
        );
